...
\begin{enumerate}
  ...
  \item \textbf{Userspace tracer and canonicalizer}: resolves successful opens through \texttt{/proc/<pid>/fd}, validates resolved targets against captured paths, retains nonblocking regular-file handles when possible, caches per-process working directories across fork for short-lived relative-path operations, filters external watch events before observation, hashes file identity as events arrive, normalizes selected system-state accesses, and attributes operations to PID-reuse-safe process lifetimes and exec epochs before assigning diagnostic logical indices.
...
\begin{itemize}
  ...
  \item structured normalization of Go import-configuration files that replaces only randomized \texttt{go-build<digits>} work-root prefixes while retaining dependency names, relative layout, and other control data,
  ...
  \item fork-aware process-instance identity: each process lifetime carries an exec epoch, children inherit tool and descriptor state, and PID reuse starts a new generation,
