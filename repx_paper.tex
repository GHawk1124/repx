...
E = (e_1, e_2, \dots, e_n),
\]
where each event is emitted from kernel tracepoints. The goal is to compute a canonical operation sequence
\[
C = \mathcal{N}(E),
\]
where $\mathcal{N}$ normalizes system-dependent details (for example, process IDs and some system-state reads), and then compute a commitment
\[
R = \mathcal{M}(C),
\]
where $\mathcal{M}$ is a Merkle-tree root over operation hashes.

Two runs are considered behaviorally equivalent under this model if and only if they produce the same root $R$ after canonicalization. A mismatch indicates behavioral divergence and can be localized by descending divergent Merkle nodes.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Userspace tracer and canonicalizer}: resolves paths, associates file descriptors, computes content hashes, normalizes selected system-state accesses, and maps real PIDs to logical process indices.
  \item \textbf{Attestation layer}: hashes canonical operations and builds a Merkle tree; emits a JSON attestation containing root hash, tree, and command metadata.
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
  \item content hashing of file inputs/outputs rather than path-only identity,
...
The Merkle commitment provides two academic benefits:
\begin{itemize}[leftmargin=*]
  \item \textbf{compact verification}: one root hash for equality checks,
  \item \textbf{explainable failure}: subtree descent to isolate where runs diverged.
\end{itemize}
...
The implementation tracks dropped ring-buffer events and distinguishes policy by mode:
\begin{itemize}[leftmargin=*]
  \item trace mode warns and still emits output,
...
\section{Conclusion}
\texttt{repx} demonstrates a practical and academically relevant pattern: kernel-level execution traces can be transformed into deterministic, cryptographically verifiable process provenance. The strongest contribution is conceptual and methodological: a concrete pathway from runtime telemetry to reproducible attestations with divergence diagnostics. While the prototype has clear limitations, it provides a strong foundation for research on process-centric supply-chain integrity and reproducible build assurance.
